...
class TCP(Protocol):
    def send_file(self, filename: str, host: str, port: int, buffer_size: int, interval: float = 0.0):
        ...
        """
        logging.info(f"TCP로 전송 시작 - 파일: {filename}")
        ...
        try:
            ...
            with open(filename, 'rb') as file:
                while True:
                    ...
                    sock.sendall(data)
                    bytes_sent += len(data)
                    logging.debug(f"전송 중: {bytes_sent}/{filesize} 바이트 ({bytes_sent / filesize * 100:.2f}%)")
                    time.sleep(interval)

            # 전송 완료 확인
            logging.info(f"파일 전송 완료: {filename} ({filesize} 바이트)")
            ...
            logging.error(f"파일 전송 중 오류 발생: {e}")
            return False
    ...
    def start_server(self, host: str, port: int, target_dir: str = "received"):
        ...
        """
        logging.info(f"TCP로 서버 시작 - {host}:{port}")

        # 저장 디렉토리 확인 및 생성
        if not os.path.exists(target_dir):
            os.makedirs(target_dir)
            logging.info(f"디렉토리 생성: {target_dir}")
        ...
        logging.info(f"서버가 {host}:{port}에서 실행 중입니다. 클라이언트 연결 대기 중...")

        try:
            while True:
                conn, addr = sock.accept()
                logging.info(f"클라이언트가 연결되었습니다: {addr}")

                try:
                    # 헤더 사이즈 수신 (4바이트 정수)
                    header = conn.recv(4)
                    if not header or len(header) != 4:
                        logging.error("유효하지 않은 헤더를 수신했습니다.")
                        conn.close()
                        continue
                    ...
                    logging.info(f"파일 수신 시작: {filename} (크기: {filesize} 바이트)")
                    ...
                    if received_size == filesize:
                        logging.info(f"파일 '{filename}'을(를) 성공적으로 수신했습니다. ({filesize} 바이트)")
                        conn.send("파일 수신 완료".encode('utf-8'))
                    else:
                        logging.warning(f"파일 '{filename}'의 수신이 불완전합니다. (예상: {filesize}, 실제: {received_size})")
